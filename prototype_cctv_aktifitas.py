...
def main():
    ...
    # varThreshold diturunkan dari 300 ke 150 agar tidak terlalu kebal/buta terhadap gerakan tangan
    ...
    while True:
        ...
        if frame_count % yolo_skip_rate == 0 or frame_count == 1:
            # conf direndahkan agar lebih sensitif ke objek jauh walau sedikit buram
        ...
        for box in last_yolo_boxes:
            ...
            # Orang dianggap AKTIF jika gerakannya >45%, ATAU sangat dekat (>40%), ATAU proporsinya berdiri (Tinggi > Lebar, aspect_ratio > 1.5)
            is_active = (motion_percentage > 45.0) or (area_percentage > 40.0) or (aspect_ratio > 1.5)
            ...
            if is_active:
                ...
                color = (0, 255, 255) # Kuning Terang (Siswa Aktif)
                
                if area_percentage > 40.0:
                    label = "Mendekati Kamera"
                    color = (0, 0, 255) # Merah (Indikasi tampering)
                elif motion_percentage > 50.0:
                    label = "Bergerak Cepat/Lari"
                elif aspect_ratio > 1.5:
                    label = "Berdiri/Jalan"
                else:
                    label = "Bergerak Aktif"
        ...
        if is_break_time:
            ...
            if active_person_class >= 15 or largest_person_area_percentage > 50.0:
                activity_severity_level = 3 
            elif active_person_class >= 11:
                activity_severity_level = 2 
            elif active_person_class >= 6:
                activity_severity_level = 1 
        else:
            # --- MODE DEFAULT (JAM BELAJAR) ---
            if active_person_class >= 5 or largest_person_area_percentage > 50.0:
                activity_severity_level = 3 
            elif active_person_class >= 3:
                activity_severity_level = 2 
            elif active_person_class >= 1:
                activity_severity_level = 1 

        total_motion_pixels = cv2.countNonZero(fgmask)

        if len(last_yolo_boxes) == 0:
            frame_category = "Aktifitas"
            frame_severity = "Kosong"
            frame_message = "Kelas Kosong / Tidak Ada Objek Terdeteksi"
            frame_color = (0, 255, 0)
        elif activity_severity_level == 0:
            frame_category = "Aktifitas"
            frame_severity = "Normal"
            frame_message = "Kelas Aman / Semua Duduk Tenang"
            frame_color = (0, 255, 0)
        else:
            frame_category = "Aktifitas"
            if activity_severity_level == 3:
                ...
                if largest_person_area_percentage > 50.0:
                    ...
                    frame_message = f"BAHAYA: Kerumunan Besar / Keributan ({active_person_class} org aktif)"
            elif activity_severity_level == 2:
                frame_severity, frame_color = "Medium", (0, 165, 255)
                frame_message = f"ALARM: Banyak Siswa Bergerak ({active_person_class} org aktif)"
            elif activity_severity_level == 1:
                frame_severity, frame_color = "Low", (0, 255, 255)
                frame_message = f"ALARM: Ada Siswa Tidak Duduk ({active_person_class} org aktif)"

        if frame_severity not in ["Normal", "Kosong"]:
